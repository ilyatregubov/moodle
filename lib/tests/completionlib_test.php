...
class core_completionlib_testcase extends advanced_testcase {
    ...
    public function test_internal_set_data() {
        ...
        $data->coursemoduleid = $cm->id;
        $c->internal_set_data($cm, $data);
        $d1 = $DB->get_field('course_completions', 'userid', array('userid' => $this->user->id));
        $this->assertEquals($d1, $data->userid);

        $data->userid = $newuser2->id;
        $c->internal_set_data($cm, $data, true);
        $d2 = $DB->get_field('course_completions', 'userid', array('userid' => $newuser2->id));
        $this->assertFalse($d2);
    }
    ...
    public function test_mark_course_completions_activity_criteria() {
        ...
        $userdata = ['userid' => $student->id, 'courseid' => $course->id];
        $coursecompletionid = mark_course_completions_activity_criteria($userdata);
        $expected = $DB->get_record('course_completions', ['userid' => $student->id]);
        $this->assertEquals($expected->id, $coursecompletionid);

        $coursecompletionid = mark_course_completions_activity_criteria();
