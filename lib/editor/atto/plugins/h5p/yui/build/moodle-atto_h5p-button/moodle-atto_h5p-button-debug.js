...
var CSS = {
    ...
    TEMPLATE = '' +
        ...
        H5PTEMPLATE = '' +
            '{{#if addParagraphs}}<p><br></p>{{/if}}' + '<div class="h5p-placeholder" contenteditable="false">' +
            '<a class="d-block h5p-placeholder-link" href="{{{url}}}">' + '{{{url}}}' +
            '</a>' + '</div>' +
...
Y.namespace('M.atto_h5p').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _getH5PDiv: function() {
        ...
        selectednodes.each(function(selNode) {
            if (selNode.hasClass('h5p-placeholder-link')) {
                H5PDiv = selNode;
            }
        });
        this._H5PDiv = H5PDiv;
