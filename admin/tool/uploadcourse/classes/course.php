                ...
                if (!isset($enrolmentdata['cohort']['cohortid'])) {
                    $errors['missingmandatoryfields'] =
                        new lang_string('missingmandatoryfields', 'tool_uploadcourse',
                            'cohortid');
                } else if (!isset($enrolmentdata['cohort']['roleid'])) {
                    $errors['missingmandatoryfields'] =
                        new lang_string('missingmandatoryfields', 'tool_uploadcourse',
                            'roleid');
                } else {
            ...
            if ($todelete) {
                ...
                if (empty($instance) && $plugin->can_add_instance($course->id)) {
                    if ($enrolmethod == 'cohort') {
                        // Have to validate second time as context was created.
                        $errors = $this->validate_cohort_enrolment_data($course->id, $enrolmentdata);
                        if ($errors) {
                            $this->error(array_key_first($errors), reset($errors));
                            break;
                        }
                        ...
                        if (!isset($enrolmentdata['cohort']['groupid'])) {
                            ...
                            $fields['customint2'] = $enrolmentdata['cohort']['groupid'];
                        }
